...
function M.GetRank(acc, totalActions)
    if totalActions == 0 then return "F", hud_utils.FSlateColor(0.4, 0, 0, 1) end -- Dark Red failure

    if acc >= 100 then return "SS", hud_utils.FSlateColor(0.69, 0.15, 1, 1) -- Electric Purple (#B026FF)
    elseif acc >= 98 then return "S+", hud_utils.FSlateColor(1, 0, 1, 1) -- Bright Magenta
    elseif acc >= 95 then return "S", hud_utils.FSlateColor(1, 1, 0, 1) -- Yellow
    elseif acc >= 90 then return "A+", hud_utils.FSlateColor(0, 1, 0.5, 1) -- Emerald Green
    elseif acc >= 85 then return "A", hud_utils.FSlateColor(0, 1, 0, 1) -- Green
    elseif acc >= 80 then return "B+", hud_utils.FSlateColor(0, 1, 1, 1) -- Cyan
    elseif acc >= 70 then return "B", hud_utils.FSlateColor(0, 0.4, 1, 1) -- Blue
    elseif acc >= 60 then return "C", hud_utils.FSlateColor(1, 0.5, 0, 1) -- Orange
    else return "D", hud_utils.FSlateColor(1, 0, 0, 1) -- Red
    end
end
...
local function renderLeftColumn(container, stats, summary)
    ...
    umg_factory.CreateTextBlock(leftVBox, "SummaryText", {
        size = 14,
        text = string.format("Overall Accuracy: %.2f%% (%d/%d Hits)", 
            displayAcc, summary.PerfectHits, summary.TotalActions),
        color = hud_utils.FSlateColor(0, 1, 1, 1) -- CYAN
    })
...
function M.Hide()
    if M.resultsWidget and M.resultsWidget:IsValid() then
        pcall(function() M.resultsWidget:SetVisibility(hud_utils.Visibility.HIDDEN) end)
    end
end
